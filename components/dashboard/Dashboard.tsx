...
import {
  ...
  Phone, Navigation, Zap, MessageSquare, ChevronDown,
} from "lucide-react";
...
type ActivityItem =
  | { kind: "estimate"; data: RecentEstimate }
  | { kind: "job"; data: RecentJob };
...
export default function Dashboard({
  business, todayJobs, recentEstimates, recentJobs, upcomingJobs, unpaidEstimates, clientsCount,
}: DashboardProps) {
  ...
  const activityFeed: ActivityItem[] = [
    ...recentEstimates.map((e): ActivityItem => ({ kind: "estimate", data: e })),
    ...recentJobs.map((j): ActivityItem => ({ kind: "job", data: j })),
  ].sort((a, b) => {
    const dateA = a.kind === "estimate" ? a.data.created_at : a.data.created_at;
    const dateB = b.kind === "estimate" ? b.data.created_at : b.data.created_at;
    return dateB.localeCompare(dateA);
  });
  const ACTIVITY_LIMIT = 4;
  const visibleActivity = activityExpanded ? activityFeed : activityFeed.slice(0, ACTIVITY_LIMIT);
    ...
    <div className="flex flex-col gap-0 pb-8">
      ...
      <div className="px-4 pt-4 pb-1">
        ...
        <p className="text-xs text-slate-400 mt-0.5">
          {todayJobs.length === 0
            ...
            : `${todayJobs.length} intervention${todayJobs.length > 1 ? "s" : ""} prévue${todayJobs.length > 1 ? "s" : ""} aujourd'hui`}
        </p>
      </div>
      ...
      <section className="px-4 pt-4">
        ...
        {activityFeed.length === 0 ? (
          ...
          <>
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden mt-2 divide-y divide-slate-50">
              {visibleActivity.map((item, i) =>
                item.kind === "estimate"
                  ? <RecentEstimateCard key={`e-${item.data.id}`} estimate={item.data} />
                  : <RecentJobRow key={`j-${item.data.id}`} job={item.data} />
              )}
            </div>
            {activityFeed.length > ACTIVITY_LIMIT && (
              <button
                onClick={() => setActivityExpanded(!activityExpanded)}
                className="w-full flex items-center justify-center gap-1.5 mt-2 py-2.5 text-xs font-semibold text-slate-500 bg-white border border-slate-200 rounded-xl active:bg-slate-50"
              >
                <ChevronDown className={cn("w-3.5 h-3.5 transition-transform", activityExpanded && "rotate-180")} />
                {activityExpanded
                  ? "Réduire"
                  : `Voir ${activityFeed.length - ACTIVITY_LIMIT} de plus`}
              </button>
            )}
          </>
        )}
...
// ─── Recent Estimate Row (inside shared container) ───────────────────────────
      ...
      <div>
        ...
        <Link href={`/devis/${e.id}/edit`} className="flex items-center gap-3 px-4 py-3.5 active:bg-slate-50">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 mb-0.5">
              <span className="text-[10px] font-black text-blue-600 uppercase tracking-wide flex-shrink-0">Devis</span>
        ...
        {(phone || locationStr) && (
          <div className="border-t border-slate-50 flex divide-x divide-slate-100">
            {phone && (
              <a href={`tel:${phone}`} className="flex items-center gap-1.5 px-3 py-2 active:bg-slate-50">
                <Phone className="w-3 h-3 text-blue-500" />
                <span className="text-[11px] font-semibold text-slate-600">Appeler</span>
              </a>
            )}
            {phone && (
              <button onClick={() => setMsgOpen(true)} className="flex items-center gap-1.5 px-3 py-2 active:bg-slate-50">
                <MessageSquare className="w-3 h-3 text-emerald-500" />
                <span className="text-[11px] font-semibold text-slate-600">Message</span>
              </button>
            )}
            {locationStr && (
              <button onClick={() => setNavOpen(true)} className="flex items-center gap-1.5 px-3 py-2 active:bg-slate-50">
...
function RecentJobRow({ job }: { job: RecentJob }) {
  const statusCfg = JOB_STATUS_CONFIG[job.status as keyof typeof JOB_STATUS_CONFIG] || { label: job.status, color: "bg-slate-100 text-slate-600" };
  return (
    <Link href={`/chantiers/${job.id}`} className="flex items-center gap-3 px-4 py-3.5 active:bg-slate-50">
      {/* Type icon */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          <span className="text-[10px] font-black text-amber-600 uppercase tracking-wide flex-shrink-0">Intervention</span>
          <span className="text-slate-200 text-[10px]">·</span>
          <span className={cn("text-[10px] font-semibold rounded px-1.5 py-0.5 flex-shrink-0", statusCfg.color)}>
            {statusCfg.label}
          </span>
        </div>
        <p className="text-sm font-semibold text-slate-900 truncate leading-tight">{job.title || "Intervention"}</p>
        <p className="text-xs text-slate-400 truncate">
          {job.client?.full_name}
