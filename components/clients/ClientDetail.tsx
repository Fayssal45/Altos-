...
import { ArrowLeft, Phone, Mail, MapPin, FileText, Plus, Edit2, MessageCircle } from "lucide-react";
...
export default function ClientDetail({ client, estimates }: ClientDetailProps) {
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex items-center gap-4">
          ...
          <div>
            ...
            {totalCA > 0 && (
              <p className="text-sm text-emerald-600 font-semibold">CA : {formatCurrency(totalCA)}</p>
            )}
            <p className="text-xs text-slate-400">{estimates.length} devis</p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          {client.phone && (
            <a href={`tel:${client.phone}`} className="flex-1 flex items-center justify-center gap-2 bg-blue-600 rounded-2xl py-3.5 text-white font-bold text-sm shadow-lg shadow-blue-600/30">
          ...
          {client.phone && (
            <a
              ...
              target="_blank"
              className="flex-1 flex items-center justify-center gap-2 bg-[#25D366] rounded-2xl py-3.5 text-white font-bold text-sm shadow-lg shadow-green-500/20"
            >
          ...
          {client.email && (
            <a href={`mailto:${client.email}`} className="flex-1 flex items-center justify-center gap-2 bg-slate-700 rounded-2xl py-3.5 text-white font-bold text-sm">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-sm font-bold text-slate-500 uppercase tracking-wider">Devis</h2>
